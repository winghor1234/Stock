...
export default function Card({ title, value, change, positive, icon, data, color, spanText }) {
    return (
        <div className="  relative bg-gradient-to-r from-[#031826] to-[#0a2a3f] border-white/5 rounded-2xl p-2 hover:bg-gradient-to-r hover:from-[#031826]/50 hover:to-[#0a2a3f]/50 transition cursor-pointer hover:shadow-[0_20px_60px_rgba(0,255,150,0.12)]">
            ...
            <h2 className="text-md text-white font-semibold">{value}</h2>

            <div className="flex gap-2 mt-1">

                {
                    positive && (
                        <span className={`text-[11px] ${positive ? "text-green-400" : "text-red-400"}`}>
                            {positive ? "▲" : "▼"} {change}
                        </span>
                    )
                }
                
                <span className="text-[11px] text-white/40">{spanText}</span>
            </div>
